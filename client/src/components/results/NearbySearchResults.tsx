...
export default function NearbySearchResults() {
  ...
  const [destinationIDValue, setDestinationIDValue] =
    useAtom(destinationIDAtom);
  const [originIDValue, setOriginIDValue] = useAtom(originIDAtom);
  const [encodedPolylineValue, setEncodedPolylineValue] =
    useAtom(encodedPolylineAtom);
  ...
  return (
    <>
      <div className="font-righteous px-4">
        Route active: {encodedPolylineValue ? "Active!" : "No route active."}
      </div>
      <div className="font-righteous px-4">
        Origin ID: {originIDValue ? originIDValue : "No origin selected."}
      </div>
      <div className="font-righteous px-4">
        Destination ID:{" "}
        {destinationIDValue ? destinationIDValue : "No destination selected."}
      </div>
        ...
        <div className="font-righteous m-4 bg-white border border-gray-200 rounded-lg shadow p-4">
          ...
          <div className="text-xl flex flex-wrap columns-3 font-fuzzy-bubbles text-gray-700 dark:text-gray-400">
            {searchResults.map((result) => (
              <div
                className=" w-full m-3 p-4 border border-gray-200 rounded-lg shadow"
                key={result.place_id}
                id={result.place_id}
              >
                <ul>
                  ...
                  <li>
                    Rating:{" "}
                    {result.rating ? result.rating : "No ratings available!"}
                  </li>
                  <li>
                    Pricing Level:{" "}
                    {result.price_level
